...
int find_sync(const uint8_t *power, int num_blocks, int num_bins,
		const uint8_t *sync_map, int num_candidates, Candidate *heap,
		int min_score) {
	int heap_size = 0;
	// int x = 500;
			...
			for (int freq_offset = ft8_min_bin; freq_offset < num_bins - 8;
				...
				for (int m = 0; m <= 72; m += 36) {
					for (int k = 0; k < 7; ++k) {
						...
						int offset = ((time_offset + k + m) * 4 + alt)
								* num_bins + freq_offset;

						const uint8_t *p8 = power + offset;

						score += 8 * p8[sync_map[k]] - p8[0] - p8[1] - p8[2]
...
void extract_likelihood(const uint8_t *power, int num_bins, Candidate cand,
		const uint8_t *code_map, float *log174) {

	int offset = (cand.time_offset * 4 + cand.time_sub * 2 + cand.freq_sub)
...
// Cleaned up code for the simple case of n_syms==1
	...
	log174[bit_idx + 0] = max4(s2[4], s2[5], s2[6], s2[7])
			- max4(s2[0], s2[1], s2[2], s2[3]);
	log174[bit_idx + 1] = max4(s2[2], s2[3], s2[6], s2[7])
			- max4(s2[0], s2[1], s2[4], s2[5]);
	log174[bit_idx + 2] = max4(s2[1], s2[3], s2[5], s2[7])
