...
void set_pll(uint64_t pll_freq, enum si5351_pll target_pll) {
	...
	//uint8_t *params = new uint8_t[20];  //chh
	uint8_t params[20];
	uint8_t i = 0;
	uint8_t temp;
 ...
 *   Set to 1 to enable, 0 to disable
 */
void set_ms(enum si5351_clock clk, Si5351RegSet ms_reg, uint8_t int_mode,
 ...
 */
void set_clock_disable(enum si5351_clock clk,
...
uint64_t pll_calc(enum si5351_pll pll, uint64_t freq, Si5351RegSet *reg,
...
void update_sys_status(Si5351Status *status) {
	uint8_t reg_val = 0;
...
void update_int_status(Si5351IntStatus *int_status) {
	uint8_t reg_val = 0;
